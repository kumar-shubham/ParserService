...
public class CIMBSGCardPDFScrapper extends PDFParser {
	...
	private Response scrapeStatement(WebDriver driver) throws Exception {
		...
		String detailsRegEx = "(\\d{2} \\w{3} \\d{4}) .*\\$((\\d*,)*\\d+(.)\\d+) .*\\$((\\d*,)*\\d+(.)\\d+) (\\d{2} \\w{3} \\d{4})";
		...
		WebElement accountEle = page.findElement(By.xpath("//td[contains(text(), 'Description of Transaction')]/../following-sibling::tr[2]"));


		String accountText = accountEle.getText().trim();
		String accountRegex = "(.*) (\\d{4}.\\d{4}.\\d{4}.\\d{4}) (.*)";
		p = Pattern.compile(accountRegex);
		m = p.matcher(accountText);
			...
			for(WebElement rowEle: transEle){
				...
				if(m1.matches()){
					...
					if(transFound){
						...
						if(rowText.contains("GRAND TOTAL")){
